...
const warningLabels: Record<string, string> = {
    ...
    DUPLICATE_PATHNAME:
        "duplicates a path already mapped (only the first is kept)",
};
...
export function ManualCompareSection({
    ...
    function setEnvironment(
        index: 0 | 1,
        patch: Partial<ManualCompareAnchorInput>,
    ): void {
    ...
    return (
        <section
            ...
            className="rounded-2xl border border-ink-200/70 bg-linear-to-br from-white to-ink-50/50 p-4"
        >
            <PlainCheckbox
                ...
                onChange={() =>
                    onChange({ ...compare, enabled: !compare.enabled })
                }
            />
            ...
            {compare.enabled ? (
                <div className="mt-4 space-y-4">
                    ...
                    {!valid ? (
                        <Notice tone="warning">
                            Choose a name and an anchor tab for each
                            environment, on two different subdomains, to enable
                            the comparison.
                        </Notice>
                    ) : null}
